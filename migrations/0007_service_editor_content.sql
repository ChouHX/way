...
SET
  ...
  overview_zh = '交通罚单种类繁多，包括 Moving Violation、Non-Moving Violation、Civil Infraction、Criminal Charge，以及可更正（Correctable）和不可更正（Non-Correctable）等类型。' || char(10) || char(10) || '在决定如何处理罚单之前，应先了解其性质及认罪可能带来的法律后果，例如驾照扣分、违规记录、保险费上涨，甚至刑事处罚。' || char(10) || char(10) || '我们会协助客户分析罚单的性质，详细解释各种处理方式及其可能产生的法律影响、成功机会和预期结果，让客户充分了解每一种选择，避免因仓促决定而承担不必要的后果。' || char(10) || char(10) || '如果客户决定对罚单提出抗辩（Contest the Ticket），我们将根据案件事实制定针对性的抗辩策略，积极维护客户的合法权益，并全程提供专业建议，帮助客户作出最符合自身利益的决定。',
  overview_en = 'Traffic tickets can take many forms, including Moving Violations, Non-Moving Violations, Civil Infractions, Criminal Charges, and correctable or non-correctable offenses.' || char(10) || char(10) || 'Before deciding how to respond, it is important to understand the nature of the ticket and the legal consequences of admitting guilt, which may include license points, a driving record, higher insurance premiums, or even criminal penalties.' || char(10) || char(10) || 'We help clients analyze the ticket, explain each available option and its potential legal effect, likelihood of success, and expected outcome, so they can make an informed decision without taking on unnecessary consequences.' || char(10) || char(10) || 'When a client decides to contest the ticket, we develop a strategy based on the facts, protect the client’s lawful interests, and provide practical guidance throughout the matter.',
