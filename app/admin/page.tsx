...
type AccessState = "loading" | "ready" | "denied";
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
      {access === "denied" ? <AccessDenied /> : <>
        ...
        {access === "loading" ? <div className="py-16 text-sm text-slate-500">正在加载配置…</div> : section === "site" ? <SiteManager site={site} setSite={setSite} saveSite={saveSite}/> : <CaseManager categories={categories} cases={cases} panel={casePanel} setPanel={setCasePanel} addCategory={addCategory} addCase={addCase}/>} 
      </>}
    </div>
  </main>;
}

function AccessDenied() { return <Card className="mx-auto mt-12 max-w-xl p-8 text-center"><span className="mx-auto grid h-11 w-11 place-items-center bg-amber-50 text-amber-700"><ShieldAlert size={21}/></span><h2 className="mt-5 text-xl font-bold text-[#0f2747]">需要 Cloudflare Access 验证</h2><p className="mt-3 text-sm leading-6 text-slate-600">此页面未收到已验证的 Cloudflare Access 身份。请确认该域名的 <code>/admin*</code> 和 <code>/api/admin*</code> 已被 Access Application 保护，并且当前登录邮箱已添加到 Allow Policy 与 <code>ADMIN_EMAILS</code>。</p></Card>; }
