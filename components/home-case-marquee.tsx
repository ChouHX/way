...
export function HomeCaseMarquee({
  ...
  const autoLoop = !category && visibleCases.length > 1;
  const loop = autoLoop ? [...visibleCases, ...visibleCases] : visibleCases;
  ...
  return (
    <div className="mt-8">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex min-w-0 gap-1.5 overflow-x-auto pb-1" aria-label={zh ? "案例分类" : "Case categories"}>
          {[{ id: "", zh: "全部案例", en: "All cases" }, ...categories].map((item) => {
            const count = item.id ? cases.filter((entry) => entry.category_id === item.id).length : cases.length;
            return <button key={item.id || "all"} type="button" aria-pressed={category === item.id} onClick={() => setCategory(item.id)} className={`shrink-0 rounded-full border px-3 py-1.5 text-[11px] font-bold transition-[background-color,border-color,color,transform] active:scale-[.97] ${category === item.id ? "border-[#1a243f] bg-[#1a243f] text-white" : "border-slate-200 bg-white text-slate-500 hover:border-[#c5b780] hover:text-[#716641]"}`}>{zh ? item.zh : item.en}<span className={`ml-1.5 tabular-nums ${category === item.id ? "text-white/65" : "text-slate-400"}`}>{count}</span></button>;
          })}
        </div>
        {autoLoop && (
          <button
      ...
      <div
        ref={viewport}
        className={category ? "overflow-x-auto py-1 pb-3" : `home-case-marquee overflow-hidden py-1 ${paused ? "is-paused" : ""}`}
      >
        <div
          key={category || "all"}
          className={`${autoLoop ? "home-case-marquee-track" : ""} flex w-max items-stretch gap-5`}
          style={{ "--marquee-duration": `${duration}s` } as CSSProperties}
        >
          {loop.map((item, index) => {
            const clone = autoLoop && index >= visibleCases.length;
            return (
              <div
                key={`${item.id}-${index}`}
