...
const content = {
  zh: {
    ...
    intro: "如果您在纽约、法拉盛、皇后区、布鲁克林、曼哈顿或长岛收到超速罚单、闯红灯罚单、手机罚单、无证驾驶传票或 E-ZPass 罚款，及时评估案件很重要。永盛罚单/移民咨询中心提供中文、英文和西班牙语服务，帮助客户了解罚款、扣分、保险上涨和出庭风险，并根据案件情况说明常见流程。",
    ...
    faqs: [
      ...
      ["法拉盛附近可以中文咨询罚单处理吗？", "可以。永盛罚单/移民咨询中心位于纽约法拉盛，提供中文、英文和西班牙语咨询，可电话、微信或到店沟通。"],
    ],
  },
  en: {
    eyebrow: "NEW YORK TICKET GUIDE",
    title: "Received a traffic ticket in New York? Start with the right information.",
    intro: "If you received a speeding ticket, red-light ticket, cellphone ticket, unlicensed-operation summons, or E-ZPass penalty in New York, Flushing, Queens, Brooklyn, Manhattan, or Long Island, a timely review matters. Yongsheng Ticket & Immigration Consulting Center provides Chinese, English, and Spanish support to help clients understand potential fines, points, insurance increases, court-related risks, and common next steps for their circumstances.",
    ...
    faqs: [
      ...
      ["When should I speak with a traffic-ticket attorney?", "If a matter involves court, substantial points, license risk, an accident, DUI, or complex facts, it may be important to understand the legal consequences further. Yongsheng can organize materials and explain common processes, but does not provide legal advice."],
      ...
      ["Can I receive Chinese-language ticket support near Flushing?", "Yes. Yongsheng Ticket & Immigration Consulting Center is located in Flushing, New York, and offers Chinese, English, and Spanish consultations by phone, WeChat, or in person."],
    ],
  },
} as const;
    ...
    <div className="mx-auto max-w-6xl px-5 py-20 md:py-24">
      <InView variants={{ hidden: { opacity: 0, transform: "translateY(10px)" }, visible: { opacity: 1, transform: "translateY(0)" } }} transition={{ duration: 0.28, ease: [0.23, 1, 0.32, 1] }}>
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.4fr)_minmax(17rem,.6fr)] lg:items-end">
          <div className="max-w-3xl"><p className="text-xs font-bold tracking-[.14em] text-blue-600">{copy.eyebrow}</p><h2 className="mt-4 text-3xl font-bold leading-[1.15] tracking-[-.03em] text-[#0f2747] md:text-4xl">{copy.title}</h2><p className="mt-6 max-w-3xl text-[1rem] leading-8 text-slate-600">{copy.intro}</p></div>
          <aside className="border-l-2 border-blue-600 bg-slate-50 px-5 py-5"><CircleAlert size={20} className="text-blue-600"/><p className="mt-3 text-sm font-semibold leading-6 text-[#0f2747]">{copy.note}</p></aside>
        </div>
      </InView>
      <div className="mt-12 grid border border-slate-200 md:grid-cols-3">{copy.cards.map(([title, description], index) => { const Icon = icons[index]; return <article key={title} className="border-b border-slate-200 p-6 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0"><div className="flex items-center gap-3"><Icon size={21} className="shrink-0 text-blue-600"/><h3 className="text-lg font-bold tracking-[-.015em] text-[#0f2747]">{title}</h3></div><p className="mt-4 text-sm leading-6 text-slate-600">{description}</p></article>; })}</div>
      <div className="mt-16 grid gap-8 lg:grid-cols-[minmax(14rem,.55fr)_minmax(0,1.45fr)] lg:gap-14"><div><p className="text-xs font-bold tracking-[.14em] text-blue-600">FAQ</p><h2 className="mt-3 text-3xl font-bold leading-tight tracking-[-.025em] text-[#0f2747]">{copy.faqTitle}</h2></div><FaqAccordion items={copy.faqs}/></div>
    </div>
  </section>;
}
