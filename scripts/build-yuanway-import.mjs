...
const translations = {
  开车手机: "Cellphone Use",
  未系安全带: "Seatbelt Violation",
  违反交通信号: "Traffic Signal Violation",
  无证驾驶: "Unlicensed Operation",
  未出示证件: "Failure to Produce Documents",
  未让行: "Failure to Yield",
  肩道行驶: "Shoulder Driving",
  超速: "Speeding",
  错误信号: "Improper Signal",
  闯红灯: "Red Light Violation",
  右侧超车: "Passing on the Right",
  违规转弯: "Improper Turn",
  暂停注册: "Suspended Registration",
  车窗抛物: "Throwing Objects from Vehicle",
  酒驾: "DWI / DWAI",
  疏忽驾驶: "Careless Driving",
  未让行人: "Failure to Yield to Pedestrian",
  欺诈相关: "Fraud-related",
  商店盗窃: "Shoplifting",
  未注册车辆: "Unregistered Vehicle",
  "妨害秩序/逃逸": "Disorderly Conduct / Evading",
  "TLC 违规": "TLC Violation",
  其他: "Other",
  纽约州: "New York State",
  布鲁克林: "Brooklyn",
  长岛: "Long Island",
...
for (const item of cases)
  if (item.type !== "其他") typeRows.set(`${item.category}|${item.type}`, item);
for (const [compound, item] of typeRows) {
  const [category, type] = compound.split("|");
  const id = `${categoryMap[category][0]}-${key(type)}`;
  sql.push(
...
regions.forEach((region, index) => {
  const id = `region-${key(region)}`;
  sql.push(
    `INSERT INTO case_regions (id,slug,name_zh,name_en,sort_order) VALUES (${q(id)},${q(slug(translations[region] ?? region) || id)},${q(region)},${q(translations[region] ?? region)},${index}) ON CONFLICT(id) DO UPDATE SET name_zh=excluded.name_zh,name_en=excluded.name_en,sort_order=excluded.sort_order;`,
  );
});

for (const guide of guides) {
  const [category, type] = guideType[guide.id];
  const categoryId = type === "其他" ? categoryMap["其他"][0] : categoryMap[category][0];
  const typeId =
    type === "其他" ? null : `${categoryId}-${key(type)}`;
  const en = guideEn[guide.id];
  sql.push(
    `INSERT INTO guides (id,slug,category_id,type_id,title_zh,title_en,summary_zh,summary_en,content_zh,content_en,source_url,published,updated_at) VALUES (${q(guide.id)},${q(guide.id)},${q(categoryId)},${q(typeId)},${q(guide.title)},${q(en[0])},${q(summaryZh(guide.content))},${q(en[1])},${q(guide.content)},${q(en[2])},${q(guide.url)},1,CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET category_id=excluded.category_id,type_id=excluded.type_id,title_zh=excluded.title_zh,title_en=excluded.title_en,summary_zh=excluded.summary_zh,summary_en=excluded.summary_en,content_zh=excluded.content_zh,content_en=excluded.content_en,source_url=excluded.source_url,published=1,updated_at=CURRENT_TIMESTAMP;`,
  );
}
for (const item of cases) {
  const categoryId =
    ...
    typeId = item.type === "其他" ? null : `${categoryId}-${key(item.type)}`,
    regionId = `region-${key(item.region)}`;
...
console.log(
  `Generated ${typeRows.size} types, ${regions.length} regions, ${guides.length} guides, and ${cases.length} cases.`,
);
